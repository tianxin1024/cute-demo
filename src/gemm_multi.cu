...
gemm_multi_stage(void *Dptr, const void *Aptr, const void *Bptr, int m, int n, int k) {
    ...
    auto sA = make_tensor(make_smem_ptr(Ashm),
                          SmemLayoutA{}); // (kTileM, kTileN, kStage)
    ...
    auto tCrD = thr_mma.partition_fragment_C(gD);          // (MMA, MMA_M, MMA_K)
...
#pragma unroll
        for (int j = 0; j < step; ++j) {
            // we add a temp tensor to cope with accumulator add output data type
            // difference
...
int main(int argc, char **argv) {
    ...
    print("\n");
    print(typename decltype(gemm_config)::MMA_EU_RepeatT{});
    print("\n");
    print(typename decltype(gemm_config)::MMA_P_T{});
    print("\n");
    print(typename decltype(gemm_config)::MMA{});
    // print_latex(typename decltype(gemm_config)::MMA{});
    print("\n");
    ...
    int shm_size = gemm_config.kShmSize; // 256
    ...
    for (int it = 0; it < nt; ++it) {
        ...
        gemm_multi_stage<decltype(gemm_config)><<<grid, block, shm_size>>>(Dptr, Aptr, Bptr, M, N, K);
    }
