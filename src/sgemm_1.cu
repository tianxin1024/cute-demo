...
__global__ static __launch_bounds__(decltype(size(CThreadLayout{}))::value) void gemm_device(ProblemShape shape_MNK, CtaTiler cta_tiler,
    ...
    CUTE_STATIC_ASSERT_V(congruent(select<0, 2>(shape_MNK), dB)); // dB strides for shape NK
    ...
    Tensor sB = make_tensor(make_smem_ptr(smemB), sA_layout); // (BLK_N, BLK_K)
    ...
    //            and the computes on those tiles.
    //    copy(.) operators on the global and shared memory via the tA|tB partitioning
    //    gemm(.) operators on the shared and register memory via the tC partitioning
...
// Use padded m-major smem sA, padded m-major smem sB, amd k-major threads tA|tB
template <class TA, class TB, class TC, class Alpha, class Beta>
