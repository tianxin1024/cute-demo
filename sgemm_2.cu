...
__global__ static __launch_bounds__(decltype(size(TiledMma{}))::value) void gemm_device(ProblemShape shape_MNK, CtaTiler cta_tiler,
    ...
    ThrMMA thr_mma = mma.get_slice(threadIdx.x); // (MMA, MMA_M, MMA_K)
    Tensor tCsA = thr_mma.partition_A(sA);       // (MMA, MMA_N, MMA_K)
...
#if 1
    // TUTORIAL: Example of an inter loop that pipelines compute with reads
    //           from global memory by staging through registers and shared memory
    //   Data is read from global to registers, then to shared via the TiledCopy partitions
    //   gemm(.)  operator on the shared memory directly via the TiledMMA partitions
