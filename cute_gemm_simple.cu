...
template <typename T>
void get_rand_data(T *data, int n);
...
int main() {
    ...
    Bptr_host = (T *)malloc(sizeof(T) * k * n);

    get_rand_data(Aptr_host, m * k);
    get_rand_data(Bptr_host, k * n);

    cudaMemcpy(Aptr, Aptr_host, sizeof(T) * m * k, cudaMemcpyHostToDevice);
    cudaMemcpy(Bptr, Bptr_host, sizeof(T) * k * n, cudaMemcpyHostToDevice);
